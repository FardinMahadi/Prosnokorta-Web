...
`;
  ...
  private generateAsyncThunks(): string {
    ...
    return this.config.asyncActions.map(action => `export const ${action} = createAsyncThunk(
  ...
  private generateReducers(): string {
    ...
    return this.config.actions.map(action => `    ${action}(state, action: PayloadAction<any>) {
      // Reducer logic here
    },`).join('\n');
  }
  ...
  private generateExtraReducers(): string {
    ...
    return this.config.asyncActions.map(action => `    builder
  ...
  private generateActionSignatures(): string {
    return this.config.actions?.map(action => `  ${action}: (payload: any) => void;`).join('\n') || '';
  }
  ...
  private generateActionImplementations(): string {
    return this.config.actions?.map(action => `    ${action}: (payload) => set((state) => ({ ${this.config.name.toLowerCase()}: payload })),`).join('\n') || '';
  }

  private generateContextActions(): string {
    return this.config.actions?.map(action => `  const ${action} = useCallback((payload: any) => {
...
`;
  ...
  private generateActionAtoms(): string {
    return this.config.actions?.map(action => `export const ${action}Atom = atom(
  null,
  (get, set, payload) => {
  ...
  private generateActionHooks(): string {
    return this.config.actions?.map(action => `export const use${this.capitalize(action)} = () => useSetRecoilState(${action}Atom);`).join('\n') || '';
  }
